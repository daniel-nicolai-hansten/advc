...
fn generate_dominoes() -> HashSet<Domino> {
    let mut dominoes = HashSet::new();
    for i in 0..=6 {
        for j in i..=6 {
            dominoes.insert((i, j).sort());
        }
    }
    dominoes
}
...
impl Puzzle {
    fn place_domino(&self, y: usize, x: usize, domino: Domino) -> Option<Self> {
        ...
        new_puzzle.remaining.remove(&domino.sort());
...
fn main() {
    ...
    for dom in &puzzle.remaining {
        println!("{:?}", dom);
    }
    if let Some(solved) = solve_puzzle(puzzle) {
...
fn solve_puzzle(puzzle: Puzzle) -> Option<Puzzle> {
    for (y, x) in (0..4).flat_map(|y| (0..7).map(move |x| (y, x))) {
        if puzzle.board[y][x].is_none() {
            for domino in &puzzle.remaining.iter().flat_map(|&d| [(d.0, d.1), (d.1, d.0)].iter()) {
                if !(puzzle.constraint_x[x].contains(&domino.0)
                    && puzzle.constraint_x[x].contains(&domino.1)
                    && puzzle.constraint_y[y].0.contains(&domino.0)
                    && puzzle.constraint_y[y].1.contains(&domino.1))
                {
                    continue;
                }
                // println!("Trying to place domino {:?} at ({}, {})", domino, y, x);
                if puzzle.constraint_y[y].0.contains(&domino.0) && puzzle.constraint_y[y].1.contains(&domino.1) {
                    // println!("Placed domino {:?} at ({}, {})", domino, y, x);
                    if let Some(new_puzzle) = puzzle.place_domino(y, x, domino) {
                        match solve_puzzle(new_puzzle) {
